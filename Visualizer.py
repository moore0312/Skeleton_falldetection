...
def plot_poseframes(data, labels=None, frames_stamp=None, delay=0.2, fig_size=(10, 5)):
    ...
    for i in range(data.shape[0]):
        ...
        if fig_cols == 2 and idx != prev_idx:
            ax2 = fig.add_subplot(1, fig_cols, 2)
            sorted_indices = np.argsort(labels[i])[::-1][:3]  # 取前3高
            sorted_values = labels[i][sorted_indices]
            sorted_classes = [CLASS_NAMES[j] for j in sorted_indices]
            ax2.bar(sorted_classes, sorted_values)
            ax2.set_ylim([0, 1.0])
